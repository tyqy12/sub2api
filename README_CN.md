...
Sub2API 内置性能监控系统，提供实时系统健康状况和性能洞察。

### 功能特性

- **请求统计** - 总请求数、活跃请求数、已完成请求数、失败请求数、超时请求数
- **响应时间指标** - 平均响应时间、P50、P95、P99 响应时间及最小/最大值
- **上游 API 耗时** - 独立追踪上游 API 响应时间
- **并发状态** - 实时用户槽位和账号槽位使用情况
- **平台分布** - 按平台（Anthropic、OpenAI、Gemini、Antigravity）的并发指标
- **分组指标** - 每个分组的并发和队列状态
- **健康检查** - 自动系统健康评估与告警
- **吞吐量监控** - 每秒请求数（RPS）追踪
...
|------|------|
| **健康** | 所有指标在正常范围内 |
| **降级** | P95 响应时间 > 60秒 或 超时率 > 1% |
| **异常** | 超时率 > 5% |

### 使用方法

从管理后台侧边栏访问性能监控页面。可随时开启/关闭监控功能，支持 30 秒间隔自动刷新实时指标。
